<section class="banner-section-two">
    <div class="banner-carousel owl-carousel owl-theme">
        <!-- Slide Item -->
        <div class="slide-item">
            <div class="bg-image" style="background-image: url(website/images/banner/hero-banner.jpeg);"></div>
            <div class="auto-container">
                <div class="content-box">
                    <div class="number animate-2">#1</div>
                    <h1 class="title animate-1">Choose Right. <br> Study Smart. </h1>
                    <div class="btn-box animate-3">
                        <a href="{{ route('about-us') }}" class="theme-btn btn-style-one"><span class="btn-title">Explore
                                now</span></a>
                        <a href="{{ route('contact-us') }}" class="theme-btn btn-style-two"><span class="btn-title">Contact
                                Us</span></a>
                    </div>
                </div>
            </div>
        </div>

        <!-- Slide Item -->
        <div class="slide-item">
            <div class="bg-image" style="background-image: url(website/images/upload/main-slider.png);"></div>
            <div class="auto-container">
                <div class="content-box">
                    <div class="number animate-2">#2</div>
                    <h1 class="title animate-1">Choose Wisely . <br> Study Smart.  </h1>
                    <div class="btn-box animate-3">
                        <a href="{{ route('about-us') }}" class="theme-btn btn-style-one"><span class="btn-title">Explore More</span></a>
                        <a href="{{ route('contact-us') }}" class="theme-btn btn-style-two"><span class="btn-title">Contact
                                Us</span></a>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="bottom-box">
        <div class="auto-container">
            <div class="content">
                <div class="inner">
                    <span class="icon icon-arrow-up"></span>
                    <div class="images">
                        <img src="{{ asset('website/images/resource/avatar-1.jpg') }}" alt="">
                        <img src="{{ asset('website/images/resource/avatar-3.jpg') }}" alt="">
                        <img src="{{ asset('website/images/resource/avatar-2.jpg') }}" alt="">
                    </div>
                    <div class="text"><strong class="color2">37800 +</strong> Students Guided in Their Academic Journey    </div>
                </div>
            </div>
        </div>
    </div>
</section>